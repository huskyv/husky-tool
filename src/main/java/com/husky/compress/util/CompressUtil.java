package com.husky.compress.util;
...
import com.husky.compress.bean.Compress;
import com.husky.compress.bean.CompressAttr;
			...
			while (fh != null) {
				...
				if (fh.isDirectory()) {
					...
					if (CheckUtil.isNotNull(compress.getDescPath())) {
						...
						if (Enums.OverWrite.OVER.getValue().equals(compress.getOverWrite())) {
							// 覆盖
							// //("【" + descPath + "】文件重名,系统自动覆盖此文件");
						} else {
							if (descFile.exists()) {
								if (Enums.OverWrite.NO_OVER.getValue().equals(compress.getOverWrite())) {
